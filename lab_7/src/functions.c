...
const char *errmsgs[] = {"Ok", "Such vertex already exists", "Graph doesn't have such vertex(vertices)", "These vertices are the same", "These vertices are not neighbors", "These vertices are already linked"};
...
const char *msgs[] = {"0. Quit", "1. Add vertex", "2. Add edge", "3. Delete vertex", "4. Show graph"};
...
int (*dialog_options[])(Graph *graph) = {NULL, dialog_create_vertex, dialog_add_edge, dialog_delete_vertex, print_graph};
...
int delete_vertex(Graph *graph, int x, int y) {
    ...
    while (cur_adj_vertex_ptr != NULL) {
        ...
        while (cur_vertex_ptr->next->id != cur_adj_vertex_id) {
            cur_vertex_ptr = cur_vertex_ptr->next;
        }
